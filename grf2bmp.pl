...
# Ukraine. (C) Демидов С.В.';
	...
	my @array_data_GRF;  # Массив с GRF-данными.

	# Распаковка GRF-данных.
	# ---
	my $addr_VCS; # Адрес для экранной области Вектора (в массиве).
	my $addr_GRF; # Адрес для GRF-данныx (в массиве).

	my $bit_s;      # 1 - повторяющиеся байты, 0 - неповторяющиеся байты.
	my $count_byte; # Счётчик для повторяющихся байт и для неповоряющихся.
	...
	my @array_data_bmp_hex; # Массив с данными (BMP-данные).
